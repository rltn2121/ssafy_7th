...
public class HouseInfoView{
	...
	private void showHouseInfo(int code) {
		...
		String dongNm = curHouse.getDong();
		
		List<StoreInfo> store = csvParser.searchByDong(dongNm);
		
		System.out.println("●●●●●●●●●● 현재 선택된 곳[HouseInfoView.java, Line: 84] ●●●●●●●●●●");
		System.out.println(curHouse);
		System.out.println("● ["+dongNm+"] 상권 개수: "+store.size() + "[HouseInfoView.java, Line: 84]");
		for(StoreInfo s : store)
...
//		});
	...
	public void setMain(){
		...
		JLabel bottomLabel = new JLabel("상권 정보", JLabel.CENTER);
		bottomP.add(bottomLabel, "North");
		frame.add(bottomP, "South");
		/*이벤트 연결*/
...
//				System.out.println("선택된 row의 column 값 :"+houseModel.getValueAt(row, 0));
	...
	private void searchHouses() {
		...
		String tmpKey = (key=="dong")?"동":"아파트 이름";
        System.out.println("[검색 분류]: "+tmpKey+", [검색어]: "+word);
		showHouses();		
	}
	/**
	 * 주택 목록을  갱신하기 위한 메서드 
